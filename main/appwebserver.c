...
void deregister_init_uris(httpd_handle_t handle)
{
  httpd_unregister_uri_handler(handle, serve_connect.uri, serve_connect.method);
  httpd_unregister_uri_handler(handle, serve_root.uri, serve_root.method);
}
...
void create_ap(void)
{
  ...
  displayPutString("SSID:");
  displaySetCursor(0, 1);
  displayPutString(REGISTER_WIFI_SSID);

  displaySetCursor(0, 4);
  displayPutString("PASS:");
  displaySetCursor(0, 5); 
  displayPutString(REGISTER_WIFI_PASS);
#endif

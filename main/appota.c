...
void do_update(char* str, size_t len)
{
  ...
  if (url == NULL) {
    ESP_LOGE(TAG, "cannot allocate URL string");
    return;
  }
  ...
  if (client == NULL) {
    ESP_LOGE(TAG, "cannot create HTTP client to download firmware");
    return;
  }
...
void check_partitions(void)
{
  ...
  if (configured_partition != NULL) {
    if (configured_partition != running_partition) {
      ESP_LOGW(TAG, "Configured OTA boot partition at offset 0x%08x, but running from offset 0x%08x", configured_partition->address, running_partition->address);
